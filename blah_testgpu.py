...
with tf.device(DEVICE):
    ...
    maxNumSequences = dataReader.get_max_len()
    x = tf.placeholder(tf.float32, [None, maxNumSequences, vecDim])
    ...
    outputs, _, _ = static_bidirectional_rnn(lstmCell_forward, lstmCell_backward,
                                          inputs=x_, dtype=tf.float32)
